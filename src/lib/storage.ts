...
export const getColorConfig = (colorValue?: string) => {
  ...
  // 如果没找到，尝试通过hex查找
  if (!color && colorValue) {
    color = CARD_COLORS.find(c => c.hex === colorValue)
  }
  
  // 如果仍然没找到，说明是自定义颜色，创建动态配置
  if (!color && colorValue && colorValue.startsWith('#')) {
    return {
      name: '自定义',
      value: colorValue,
      hex: colorValue,
  ...
  return color || CARD_COLORS[0] // 默认天空蓝
}
...
const runMigration = (): void => {
  try {
    ...
    console.log('Running data migration to version', CURRENT_VERSION)
    ...
    console.log('Migration to v3.0 completed: removed individual colors')
  } catch (error) {
